...
public class SettlementService {
    ...
    public void allocateToStoresPending(CustomerOrder order) {
        // ✅ FIX: Dùng amountCharged (số tiền thực tế khách trả) thay vì lineTotal (giá gốc)
        Map<UUID, BigDecimal> storeTotals = order.getItems().stream()
                .collect(Collectors.groupingBy(
                        CustomerOrderItem::getStoreId,
                        Collectors.mapping(item -> {
                            // Ưu tiên amountCharged, fallback sang finalLineTotal, cuối cùng là lineTotal
                            BigDecimal charged = item.getAmountCharged();
                            if (charged == null) {
                                charged = item.getFinalLineTotal();
                            }
                            if (charged == null) {
                                charged = item.getLineTotal();
                            }
        ...
        for (Map.Entry<UUID, BigDecimal> e : storeTotals.entrySet()) {
            ...
            BigDecimal amount = e.getValue(); // ✅ Đây giờ là số tiền thực tế (đã trừ discount)
