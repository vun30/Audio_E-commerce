...
public class StoreWalletDebtCron {
    ...
    public void recalcStoreDebtBalance() {
        ...
        log.info("StoreWalletDebtCron started execution at {}", java.time.LocalDateTime.now());
        ...
        log.info("StoreWalletDebtCron calculated debt for {} stores", debtMap.size());
        ...
        for (Map.Entry<UUID, BigDecimal> entry : debtMap.entrySet()) {
            ...
            if (currentDebt == null || currentDebt.compareTo(newDebt) != 0) {
        ...
        if (updated > 0) {
            ...
            log.info("StoreWalletDebtCron completed with no updates needed");
        }
    }
